...
class BaseAddress(object):
    ...
    def _decode(self, address):
        self._decoded = bytearray(unhexlify(addressvalidation.decode(address)))
    ...
    def __repr__(self):
        return addressvalidation.encode(hexlify(self._decoded))
...
class Address(BaseAddress):
    ...
    def with_payment_id(self, payment_id=0):
        ...
        return IntegratedAddress(addressvalidation.encode(hexlify(data + checksum)))
...
class IntegratedAddress(Address):
    ...
    def base_address(self):
        ...
        return Address(addressvalidation.encode(hexlify(data + checksum)))
...
def address(addr, label=None):
    ...
    if _ADDR_REGEX.match(addr):
        netbyte = bytearray(unhexlify(addressvalidation.decode(addr)))[0]
