...
from Bot import logchanbot, EMOJI_ERROR, EMOJI_RED_NO, EMOJI_INFORMATION, num_format_coin, seconds_str, RowButton_row_close_any_message, SERVER_BOT, EMOJI_HOURGLASS_NOT_DONE, DEFAULT_TICKER, text_to_num
...
# Tip
async def add_memetip(meme_id: str, owner_userid: str, from_userid: str, guild_id: str, channel_id: str, real_amount: float, coin: str, token_decimal: int, contract: str, real_amount_usd: float):
    try:
        ...
        async with store.pool.acquire() as conn:
            async with conn.cursor() as cur:
                sql = """ INSERT INTO `meme_tipped` (`meme_id`, `owner_userid`, `tipped_by`, `tipped_amount`, `real_amount_usd`, `tipped_coin`, `tipped_guild`, 
                          ...
                          """
                await cur.execute(sql, ( meme_id, owner_userid, from_userid, real_amount, real_amount_usd, TOKEN_NAME, guild_id, channel_id, currentTs, tipped_comment,                   meme_id, 
                                   TOKEN_NAME, contract, from_userid, to_userid, guild_id, channel_id, real_amount, real_amount_usd, token_decimal, tiptype, currentTs, user_server, from_userid, TOKEN_NAME, user_server, -real_amount, currentTs, to_userid, TOKEN_NAME, user_server, real_amount, currentTs ))
                await conn.commit()
                return cur.rowcount
...
class TipOtherCoin(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        try:
            ...
            tip = await add_memetip(self.meme_id, self.owner_userid, str(interaction.author.id), guild_id, channel_id, amount, COIN_NAME, coin_decimal, contract, amount_in_usd)
            if tip > 0:
                try:
                    msg = "A user has tipped to your meme `{}` with amount `{} {}`. Cheers!".format(self.meme_id, amount, COIN_NAME)
                ...
                try:
                    ...
                    embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar)
                    embed.set_image(url=self.meme_web_path + self.get_meme['saved_name'])
                    embed.set_footer(text="Tipped by: {}#{}".format(interaction.author.name, interaction.author.discriminator))
            ...
            try:
                ...
                if tip > 0:
                    ...
                    try:
                        ...
                        embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar)
                        embed.set_image(url=self.meme_web_path + self.get_meme['saved_name'])
                        embed.set_footer(text="Tipped by: {}#{}".format(interaction.author.name, interaction.discriminator))
                        channel = self.bot.get_channel(self.meme_tip_channel)
                        await channel.send(embed=embed)
                    except Exception as e:
                        traceback.print_exc(file=sys.stdout)
...
class MemeReview_Button(disnake.ui.View):
    ...
    async def click_memepls_approve(
        ...
        if interaction.author.id != self.ctx.author.id:
            await interaction.response.send_message(content=f"{interaction.author.mention}, that's not your reviewed item.")
            return
        ...
        try:
            ...
            if toggle > 0:
                await interaction.response.send_message(content=f"{interaction.author.mention}, successfully approved `{self.meme_id}`.")
                ...
                await interaction.response.send_message(content=f"{interaction.author.mention}, failed to approve `{self.meme_id}` or it is already approved.")
        except Exception as e:
            traceback.print_exc(file=sys.stdout)
        original_message = await self.ctx.original_message()
        await original_message.edit(view=None)
    ...
    async def click_memepls_reject(
        ...
        try:
            ...
            if toggle > 0:
                await interaction.response.send_message(content=f"{interaction.author.mention}, successfully rejected `{self.meme_id}`.")
            else:
                await interaction.response.send_message(content=f"{interaction.author.mention}, failed to reject `{self.meme_id}` or it is already not approved.")
        except Exception as e:
            traceback.print_exc(file=sys.stdout)
        original_message = await self.ctx.original_message()
        await original_message.edit(view=None)
...
class MemePls(commands.Cog):
    ...
    async def save_uploaded(self, key: str, owner_userid: str, owner_name: str, guild_id: str, channel_id: str, caption: str, original_name: str, saved_name: str, file_type: str, sha256: str, uploaded_date: int):
        try:
            ...
            async with self.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(sql, ( key, owner_userid, owner_name, guild_id, channel_id, caption, original_name, saved_name, file_type, sha256, uploaded_date ))
                    await conn.commit()
                    return cur.rowcount
        except Exception as e:
            traceback.print_exc(file=sys.stdout)
    ...
    async def meme_view(self, ctx):
        ...
        if get_meme is None:
            await ctx.edit_original_message(content=f"{ctx.author.mention}, cound not get one random meme yet. Try again later!")
            return
        else:
            embed = disnake.Embed(title="MEME uploaded by {}".format(get_meme['owner_name']), description=f"You can tip from your balance.", timestamp=datetime.now())
            embed.add_field(name="View Count", value=get_meme['number_view']+1, inline=False)
    ...
    async def meme_user(self, ctx, member):
        try:
            ...
            traceback.print_exc(file=sys.stdout)
            await ctx.response.send_message(f"{ctx.author.mention}, error during checking user's meme...", ephemeral=True)
            return
        if hasattr(ctx, "guild") and hasattr(ctx.guild, "id"):
            try:
                serverinfo = await store.sql_info_by_server(str(ctx.guild.id))
        ...
        if get_meme is None:
            await ctx.edit_original_message(content=f"{ctx.author.mention}, cound not get his/her random meme yet. Try again later or ask them to upload more?!")
            return
        else:
            embed = disnake.Embed(title="MEME uploaded by {}".format(get_meme['owner_name']), description=f"You can tip from your balance.", timestamp=datetime.now())
            embed.add_field(name="View Count", value=get_meme['number_view']+1, inline=False)
    ...
    async def meme_review(self, ctx, meme_id):
        ...
        try:
            try:
                ...
                traceback.print_exc(file=sys.stdout)
                await ctx.response.send_message(f"{ctx.author.mention}, error during checking meme ID...", ephemeral=True)
                return
            ...
            if meme_id is None:
                ...
                if get_meme is None:
                    ...
                    if get_meme['enable'] == 1:
                        status = "APPROVED"
                    embed = disnake.Embed(title="MEME uploaded by {} / {} in {}".format(get_meme['owner_name'], get_meme['owner_userid'], get_meme['guild_id']), description=f"Status `{status}`", timestamp=datetime.now())
                    ...
                    embed.add_field(name="New URL link", value=self.meme_web_path + get_meme['saved_name'], inline=False)
                    ...
                    embed.set_footer(text="Reviewing {} by: {}#{}".format(get_meme['key'], ctx.author.name, ctx.author.discriminator))
                    try:
                    ...
                    async with session.get(str(attachment), timeout=32) as response:
                        if response.status == 200:
                            ...
                            if mime_type not in self.meme_accept:
                                ...
                                saving = await self.save_uploaded(random_string, str(ctx.author.id), "{}#{}".format(ctx.author.name, ctx.author.discriminator), guild_id, channel_id, caption, original_name, saved_name, mime_type, hex_dig, int(time.time()))
                                if saving > 0:
                                    embed = disnake.Embed(title="MEME uploaded by {}#{} / {} in {}".format(ctx.author.name, ctx.author.discriminator, ctx.author.id, guild_name), description=f"Please help to check and review `{random_string}`!", timestamp=datetime.now())
                                    ...
                                    embed.add_field(name="New URL link", value=self.meme_web_path + saved_name, inline=False)
                                    embed.set_thumbnail(url=str(attachment))
                                    embed.set_footer(text="Posted by: {}#{}".format(ctx.author.name, ctx.author.discriminator))
