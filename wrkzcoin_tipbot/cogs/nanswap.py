        ...
        async with store.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if coin_family == "NANO":
                    ...
                    `content`, `time_insert`, `user_server`, `network`, `blockNumber`, `confirmed_depth`, `status`) 
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
                    """
                    data_rows += [
                        coin_name, contract,
                        user_id, from_address, to_address, amount,
                        0.0, decimal, tx_hash, None, int(time.time()),
...
async def nanswap_create_order(
    ...
    contract: str=None, network: str=None, contents: str=None
):
    try:
        await store.openConnection()
        async with store.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if tx_hash is not None:
                    if coin_family == "NANO":
                        ...
                        """
                        data_rows += [
                            coin_name, contract, user_id, from_amount, fee, from_decimal,
                            to_address, int(time.time()), tx_hash, contents, user_server, network
                        ]
                    elif coin_family == "XLM":
                        sql += """
            ...
            if get_limit is None:
                ...
                if truncate(actual_balance, 12) < truncate(amount + Decimal(sell_net_fee), 12):
                    ...
                    if get_estimate is None:
                        ...
                        if main_address is None and for_type_coin not in ["BTC", "ERC-20", "XTZ"]:
                        ...
                        if view.value is None:
                            ...
                            if trade is None:
                                    ...
                                    if send_tx is None:
                                        ...
                                        await ctx.edit_original_message(
                                            content=f"{ctx.author.mention}, successfully created an order from "\
                                                f"{sell_coin_emoji} __{num_format_coin(amount)} {sell_token}__ "\
                                ...
                                inserting = await nanswap_create_order(
                                    ...
                                    fee, tx_key, sell_contract, sell_network, contents
                                )
    ...
    async def nanswap_buy(
        ...
        try:
            ...
            if amount < Decimal(real_min_deposit):
                msg = f"{EMOJI_RED_NO} {ctx.author.mention}, the receiving amount is less "\
                    f"than minimum deposit __{num_format_coin(amount)} < {num_format_coin(real_min_deposit)}__ {for_token}!"
                await ctx.edit_original_message(content=msg)
                return
            ...
            if get_estimate is None:
                ...
                if get_limit is None:
                    ...
                    if get_limit['max'] is None:
                        max_limit = "unlimited"
                    msg = f"{EMOJI_RED_NO} {ctx.author.mention}, your order amount in Nanswap is not in limit range "\
                        f"{str(get_limit['min'])} - {max_limit} {sell_token}. Estimation got {num_format_coin(amount_sell)} {sell_token}!"
                    await ctx.edit_original_message(content=msg)
                    return
                            ...
                            if trade is None:
                                ...
                                if sell_token in ["BAN", "XDG", "XNO"]:
                                    ...
                                    if send_tx is None:
                                        ...
                                        await ctx.edit_original_message(
                                            content=f"{ctx.author.mention}, successfully created an order from "\
                                                f"__{num_format_coin(amount_sell)} {sell_token}__ to __{for_token}__. You shall be credited very soon! Nanswap ID: {trade['id']}.", view=None)
                                        await log_to_channel(
                                            "nanswap",
                                            f"[NANSWAP] User {ctx.author.mention} successfully created an order "\
                ...
                try:
                    ...
                    if check_id and check_id['status'] == "completed":
                        ...
                        if coin_family == "XLM":
                            ...
                            rpchost = getattr(getattr(self.bot.coin_list, coin_family), "rpchost")
                            net_name = getattr(getattr(self.bot.coin_list, coin_family), "net_name")
                            confirmation_depth = getattr(getattr(self.bot.coin_list, i['to_coin']), "deposit_confirm_depth")
                            get_tx = await tezos_get_tx(rpchost, check_id['payoutHash'], 32)
                            if get_tx is None:
                                continue
                            else:
                                height = await self.wallet_api.get_block_height(coin_family, "XTZ", net_name)
                                if height and height - get_tx['level'] > confirmation_depth:
                                    status_tx = "CONFIRMED"
                                    confirmations = height - get_tx['level']
                                else:
                                    continue
                        elif coin_family == "ERC-20":
                            # check tx status
