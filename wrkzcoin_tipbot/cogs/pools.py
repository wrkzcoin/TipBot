    ...
    async def get_miningpool_coinlist(self):
        ...
        try:
            async with aiohttp.ClientSession() as cs:
                async with cs.get(config.miningpoolstat.coinlist_link + "??timestamp=" + str(int(time.time())),
                                  timeout=config.miningpoolstat.timeout) as r:
                    if r.status == 200:
                        ...
                        async with cs.get(link, timeout=config.miningpoolstat.timeout) as r:
                            if r.status == 200:
                                ...
                                if decoded_data and len(decoded_data) > 0 and 'data' in decoded_data:
                                    redis_utils.redis_conn.set(key, json.dumps(decoded_data),
                                                               ex=config.miningpoolstat.expired)
                                    return decoded_data
                                else:
                    ...
                    if len(get_pool_data['data']) == 0:
                            ...
                            for each_pool in pool_list:
                                ...
                                if pool_name is None:
                                    pool_name = each_pool['url'].replace("https://", "").replace("http://", "").replace(
                                        "www", "")
                                pool_links += "#{}. [{}]({}) - {} __{}__\n".format(i, pool_name, each_pool['url'],
                        ...
                        try:
                            ...
                            await self.sql_miningpoolstat_fetch(coin_name, str(ctx.author.id),
                                                                ...
                                                                str(ctx.guild.id) if hasattr(ctx, "guild") and hasattr(
                                                                    ctx.guild, "id") else "DM",
                                                                ctx.guild.name if hasattr(ctx, "guild") and hasattr(
                                                                    ctx.guild, "name") else "DM",
                        ...
                        try:
                            ...
                            for each_pool in pool_list:
                                if num_pool == 0 or num_pool % pool_nos_per_page == 0:
                                    ...
                                    if 'hashrate' in get_pool_data:
                                        page.add_field(name="Hashrate", value=self.hhashes(get_pool_data['hashrate']),
                                                       inline=True)
                                ...
                                if num_pool > 0 and num_pool % pool_nos_per_page == 0:
                                    ...
                                    page.add_field(name="OTHER LINKS",
                                                   value="{} / [Invite TipBot]({}) / [Support Server]({}) / [TipBot Github]({})".format(
                                                       ...
                                                       config.discord.support_server_link, config.discord.github_link),
                                                   inline=False)
                                    page.set_footer(
                            ...
                            try:
                                ...
                                await self.sql_miningpoolstat_fetch(coin_name, str(ctx.author.id),
                                                                    ...
                                                                    str(ctx.guild.id) if hasattr(ctx,
                                                                                                 "guild") and hasattr(
                                                                        ctx.guild, "id") else "DM",
                                                                    ctx.guild.name if hasattr(ctx, "guild") and hasattr(
                                                                        ctx.guild, "name") else "DM",
                            ...
                            if redis_utils.redis_conn.exists(key_p):
                                ...
                                try:
                                            ...
                                            if i < 15 and i < len(result):
                                                if len(each) >= 4:
                                                    ...
                                                    if [ele for ele in hash_list if
                                                        ...
                                                        hash_rate = ''
                                                    if hash_rate == '' and len(each) >= 5 and [ele for ele in hash_list
                                                                                               if ((ele in each[
                                                            4]) and ('Hashrate' not in each[4]))]:
                                                        hash_rate = each[4]
                                                    elif hash_rate == '' and len(each) >= 6 and [ele for ele in
                                                                                                 hash_list if ((ele in
                                                                                                                each[
                                                                                                                    5]) and (
                                                                                                                       'Hashrate' not in
                                                                                                                       each[
                                                                                                                           5]))]:
                                                        hash_rate = each[5]
                                                    elif hash_rate == '' and len(each) >= 7 and [ele for ele in
                                                                                                 hash_list if ((ele in
                                                                                                                each[
                                                                                                                    6]) and (
                                                                                                                       'Hashrate' not in
                                    ...
                                    await self.sql_miningpoolstat_fetch(coin_name, str(ctx.author.id),
                                                                        '{}#{}'.format(ctx.author.name,
                                                                                       ctx.author.discriminator),
                                                                        requested_date, respond_date,
                                                                        json.dumps(result),
                                                                        str(ctx.guild.id) if hasattr(ctx,
                                                                                                     "guild") and hasattr(
                                                                            ctx.guild, "id") else "DM",
                                                                        ctx.guild.name if hasattr(ctx,
                                                                                                  "guild") and hasattr(
                                                                            ctx.guild, "name") else "DM",
                                                                        str(ctx.channel.id), is_cache, SERVER_BOT,
                                                                        'YES')
                                    break
                                    if ctx.author.id in self.bot.MINGPOOLSTAT_IN_PROCESS:
                                        self.bot.MINGPOOLSTAT_IN_PROCESS.remove(ctx.author.id)
