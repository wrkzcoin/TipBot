...
from Bot import logchanbot, EMOJI_ERROR, EMOJI_CHECKMARK, EMOJI_INFORMATION
import store
...
class SomeRandomAPI(commands.Cog):
    ...
    async def add_animal_db(self, image_url: str, local_path: str, sha256: str, requested_by_uid: str, requested_by_name: str, jsondump: str):
        try:
            await store.openConnection()
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(sql, ( image_url, local_path, sha256, jsondump, requested_by_uid, requested_by_name, int(time.time()) ))
    ...
    async def animal(
        ...
        try:
            ...
            traceback.print_exc(file=sys.stdout)
            await ctx.response.send_message(f"{EMOJI_INFORMATION} {ctx.author.mention}, failed to command random api...", ephemeral=True)
            return
        try:
            ...
            if fetch:
                if "image" in fetch:
                    embed = disnake.Embed(title=f"Animal {name}", description=f"Random Animal", timestamp=datetime.now())
                    fact = None
                    if "fact" in fetch:
                        embed.add_field(name="Fact", value=fetch['fact'], inline=False)
                        fact = fetch['fact']
                    embed.set_image(url=fetch["image"])
                    embed.set_footer(text=f"Requested by {ctx.author.name}#{ctx.author.discriminator} | Powered by {self.poweredby}")
                    ...
                    try:
                        ...
                        if check is None:
                            ...
                            if fetch_img:
                                add = await self.add_animal_db(fetch["image"], fetch_img['saved_location'], fetch_img['sha256'], str(ctx.author.id), "{}#{}".format(ctx.author.name, ctx.author.discriminator), json.dumps(fetch) )
                    except Exception as e:
                        traceback.print_exc(file=sys.stdout)
                else:
    ...
    async def fact(
        ...
        try:
            ...
            traceback.print_exc(file=sys.stdout)
            await ctx.response.send_message(f"{EMOJI_INFORMATION} {ctx.author.mention}, failed to command random fact api...", ephemeral=True)
            return
        try:
            ...
            if fetch:
                if "fact" in fetch:
                    ...
                    # Insert DB # Check if DB exists
                    try:
                        add = await self.add_fact_db( name, fact, str(ctx.author.id), "{}#{}".format(ctx.author.name, ctx.author.discriminator) )
                    except Exception as e:
                        traceback.print_exc(file=sys.stdout)
                else:
                    error = disnake.Embed(title=":exclamation: Error", description=" :warning: internal error!")
    ...
    async def image(
        ...
        try:
            ...
            traceback.print_exc(file=sys.stdout)
            await ctx.response.send_message(f"{EMOJI_INFORMATION} {ctx.author.mention}, failed to command random image api...", ephemeral=True)
            return
        try:
                ...
                if "link" in fetch:
                    ...
                    try:
                        ...
                        if check is None:
                            ...
                            if fetch_img:
                                add = await self.add_animal_db(fetch["link"], fetch_img['saved_location'], fetch_img['sha256'], str(ctx.author.id), "{}#{}".format(ctx.author.name, ctx.author.discriminator), json.dumps(fetch) )
                    except Exception as e:
                        traceback.print_exc(file=sys.stdout)
                else:
                    error = disnake.Embed(title=":exclamation: Error", description=" :warning: internal error!")
    ...
    async def joke(
        ...
        try:
            ...
            traceback.print_exc(file=sys.stdout)
            await ctx.response.send_message(f"{EMOJI_INFORMATION} {ctx.author.mention}, failed to command random joke api...", ephemeral=True)
            return
        try:
            ...
            if fetch:
                if "joke" in fetch:
                    ...
                    await ctx.edit_original_message(content=None, embed=embed)
                    try:
                        add = await self.add_joke_db( joke, str(ctx.author.id), "{}#{}".format(ctx.author.name, ctx.author.discriminator) )
                    except Exception as e:
                        traceback.print_exc(file=sys.stdout)
                else:
                    error = disnake.Embed(title=":exclamation: Error", description=" :warning: internal error!")
