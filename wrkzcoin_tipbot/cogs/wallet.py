...
class Wallet(commands.Cog):
                                        ...
                                        if moving:
                                            ...
                                            async with self.pool.acquire() as conn:
                                                async with conn.cursor() as cur:
                                                    ...
                                                    await cur.execute(sql, ( COIN_NAME, contract, each_addr['user_id'], each_addr['balance_wallet_address'], config.sol.MainAddress, actual_balance - tx_fee, real_deposit_fee, coin_decimal, moving, int(time.time()), each_addr['user_server'], int(time.time()), each_addr['balance_wallet_address'] ))
                                                    await conn.commit()
                            except Exception as e:
                                traceback.print_exc(file=sys.stdout)
        except Exception as e:
                                        ...
                                        try:
                                            if tx['hash'] not in d:
                                                ...
                                                if 'paymentID' in tx and len(tx['paymentID']) > 0:
                                                    try:
                                                        user_paymentId = await store.sql_get_userwallet_by_paymentid( tx['paymentID'], COIN_NAME, getattr(getattr(self.bot.coin_list, COIN_NAME), "type") )
                                                        u_server = None
                                                        ...
                                                        sql = """ INSERT IGNORE INTO `cn_get_transfers` (`coin_name`, `txid`, `payment_id`, `height`, `timestamp`, `amount`, `fee`, `decimal`, `address`, `time_insert`, `user_server`) 
                                                                  VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) """
                                                        await cur.execute(sql, (COIN_NAME, tx['hash'], tx['paymentID'], tx['blockHeight'], tx['timestamp'], float(int(tx['transfers'][0]['amount'])/10**coin_decimal), float(int(tx['fee'])/10**coin_decimal), coin_decimal, address, int(time.time()), u_server) )
                                                        await conn.commit()
                                                        ...
                                                        sql = """ INSERT IGNORE INTO `discord_notify_new_tx` (`coin_name`, `txid`, `payment_id`, `height`, `amount`, `fee`, `decimal`, `user_server`) 
                                                                  VALUES (%s, %s, %s, %s, %s, %s, %s, %s) """
                                                        await cur.execute(sql, ( COIN_NAME, tx['hash'], tx['paymentID'], tx['blockHeight'], float(int(tx['transfers'][0]['amount'])/10**coin_decimal), float(int(tx['fee'])/10**coin_decimal), coin_decimal, u_server ))
                                                        await conn.commit()
                                                    except Exception as e:
                                                        traceback.print_exc(file=sys.stdout)
                                        except Exception as e:
                                                ...
                                                if tx['transactionHash'] not in d:
                                                    ...
                                                    if 'paymentId' in tx and len(tx['paymentId']) > 0:
                                                        try:
                                                            ...
                                                            sql = """ INSERT IGNORE INTO `cn_get_transfers` (`coin_name`, `txid`, `payment_id`, `height`, `timestamp`, `amount`, `fee`, `decimal`, `address`, `time_insert`, `user_server`) 
                                                                      VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) """
                                                            await cur.execute(sql, ( COIN_NAME, tx['transactionHash'], tx['paymentId'], tx['blockIndex'], tx['timestamp'], float(tx['amount']/10**coin_decimal), float(tx['fee']/10**coin_decimal), coin_decimal, address, int(time.time()), u_server ))
                                                            await conn.commit()
                                                            # add to notification list also
                                                            sql = """ INSERT IGNORE INTO `discord_notify_new_tx` (`coin_name`, `txid`, `payment_id`, `height`, `amount`, `fee`, `decimal`, `user_server`) 
                                                                      VALUES (%s, %s, %s, %s, %s, %s, %s, %s) """
                                                            await cur.execute(sql, ( COIN_NAME, tx['transactionHash'], tx['paymentId'], tx['blockIndex'], float(tx['amount']/10**coin_decimal), float(tx['fee']/10**coin_decimal), coin_decimal, u_server ))
                                                            await conn.commit()
                                                        except Exception as e:
                                                            traceback.print_exc(file=sys.stdout)
                                            except Exception as e:
        ...
        try:
            ...
            if len(list_xmr_api) > 0:
                ...
                for COIN_NAME in list_coins:
                    ...
                    get_transfers = await self.wallet_api.call_aiohttp_wallet_xmr_bcn('get_transfers', COIN_NAME, payload=payload)
                                        ...
                                        if height >= int(tx['height']) + get_confirm_depth and tx['amount'] >= get_min_deposit_amount and 'payment_id' in tx:
                                            ...
                                            try:
                                                if tx['txid'] not in d:
                                                    ...
                                                    user_paymentId = await store.sql_get_userwallet_by_paymentid( tx['payment_id'], COIN_NAME, getattr(getattr(self.bot.coin_list, COIN_NAME), "type") )
                                                    u_server = None
                                                    if user_paymentId: u_server = user_paymentId['user_server']
                                                    sql = """ INSERT IGNORE INTO `cn_get_transfers` (`coin_name`, `in_out`, `txid`, `payment_id`, `height`, `timestamp`, `amount`, `fee`, `decimal`, `address`, `time_insert`, `user_server`) 
                                                              VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) """
                                                    await cur.execute(sql, ( COIN_NAME, tx['type'].upper(), tx['txid'], tx['payment_id'], tx['height'], tx['timestamp'], float(tx['amount']/10**coin_decimal), float(tx['fee']/10**coin_decimal), coin_decimal, tx_address, int(time.time()), u_server ) )
                                                    await conn.commit()
                                                    # add to notification list also
                                                    sql = """ INSERT IGNORE INTO `discord_notify_new_tx` (`coin_name`, `txid`, `payment_id`, `height`, `amount`, `fee`, `decimal`, `user_server`) 
                                                              VALUES (%s, %s, %s, %s, %s, %s, %s, %s) """
                                                    await cur.execute(sql, ( COIN_NAME, tx['txid'], tx['payment_id'], tx['height'], float(tx['amount']/10**coin_decimal), float(tx['fee']/10**coin_decimal), coin_decimal, u_server ) )
                                                    await conn.commit()
                                            except Exception as e:
                                                traceback.print_exc(file=sys.stdout)
                                                    ...
                                                    if COIN_NAME in ["PGO", "PIVX"]:
                                                        ...
                                                        if tx['category'] == 'receive' and 'generated' not in tx:
                                                            sql = """ INSERT IGNORE INTO `doge_get_transfers` (`coin_name`, `txid`, `blockhash`, `address`, `blocktime`, `amount`, `fee`, `confirmations`, `category`, `time_insert`, `user_server`) 
                                                                      VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) """
                                                            await cur.execute(sql, ( COIN_NAME, tx['txid'], tx['blockhash'], tx['address'], tx['blocktime'], float(tx['amount']), float(tx['fee']) if 'fee' in tx else None, tx['confirmations'], tx['category'], int(time.time()), u_server ) )
                                                            await conn.commit()
                                                            # Notify Tx
                                                        if (tx['amount'] > 0) and tx['category'] == 'receive' and 'generated' not in tx:
                                                            sql = """ INSERT IGNORE INTO `discord_notify_new_tx` (`coin_name`, `txid`, `payment_id`, `blockhash`, `amount`, `fee`, `decimal`, `user_server`) 
                                                                      VALUES (%s, %s, %s, %s, %s, %s, %s, %s) """
                                                            await cur.execute(sql, ( COIN_NAME, tx['txid'], tx['address'], tx['blockhash'], float(tx['amount']), float(tx['fee']) if 'fee' in tx else None, coin_decimal, u_server ))
                                                            await conn.commit()
                                                    else:
                                                        # generate from mining
                                            ...
                                            if height >= get_confirm_depth + int(tx['confirmed_at_height']) and tx['amount'] >= get_min_deposit_amount:
                                                ...
                                                try:
                                                    if tx['name'] not in d:
                                                        # receive
                                                        if len(tx['sent_to']) == 0:
                                                            user_paymentId = await store.sql_get_userwallet_by_paymentid( tx['to_address'], COIN_NAME, getattr(getattr(self.bot.coin_list, COIN_NAME), "type") )
                                                            u_server = None
                                                            if user_paymentId: u_server = user_paymentId['user_server']
                                                            ...
                                                            await cur.execute(sql, ( COIN_NAME, tx['name'], tx['confirmed_at_height'], tx['created_at_time'],
                                                                                    tx['to_address'], float(tx['amount']/10**coin_decimal), float(tx['fee_amount']/10**coin_decimal), coin_decimal, int(time.time()), u_server ))
                                                            await conn.commit()
                                                        # add to notification list also, doge payment_id = address
                                                        if (tx['amount'] > 0) and len(tx['sent_to']) == 0:
                                                            ...
                                                            await cur.execute(sql, (COIN_NAME, tx['name'], tx['to_address'], tx['name'], int(tx['confirmed_at_height']), 
                                                                                    float(tx['amount']/10**coin_decimal), float(tx['fee_amount']/10**coin_decimal), coin_decimal, u_server))
                                                            await conn.commit()
                                                except Exception as e:
                                                    traceback.print_exc(file=sys.stdout)
                                            if height < get_confirm_depth + int(tx['confirmed_at_height']) and tx['amount'] >= get_min_deposit_amount:
                        ...
                        for address, balance in get_balance.items():
                            try:
                                ...
                                if float(int(balance['balance'])/10**coin_decimal) >= real_min_deposit and float(int(balance['pending'])/10**coin_decimal) == 0 and address in all_deposit_address_keys:
                                    ...
                                    try:
                                        main_address = getattr(getattr(self.bot.coin_list, COIN_NAME), "MainAddress")
                                        move_to_deposit = await self.wallet_api.nano_sendtoaddress(address, main_address, int(balance['balance']), COIN_NAME) # atomic
                                        # add to DB
                                        if move_to_deposit:
                                            try:
                                                await self.openConnection()
                                                async with self.pool.acquire() as conn:
                                                    async with conn.cursor() as cur:
                                                        sql = """ INSERT INTO nano_move_deposit (`coin_name`, `user_id`, `balance_wallet_address`, `to_main_address`, `amount`, `decimal`, `block`, `time_insert`, `user_server`) 
                                                                  VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) """
                                                        await cur.execute(sql, ( COIN_NAME, all_deposit_address[address]['user_id'], address, main_address, float(int(balance['balance'])/10**coin_decimal), coin_decimal, move_to_deposit['block'], int(time.time()), all_deposit_address[address]['user_server'] ))
                                                        await conn.commit()
                                                        updated += 1
                                                        # add to notification list also
                                                        # txid = new block ID
        ...
        try:
            ...
            if len(erc_contracts) > 0:
                for each_c in erc_contracts:
                    try:
                        ...
                        await store.trx_check_minimum_deposit(each_c['coin_name'], type_name, each_c['contract'], each_c['decimal'], each_c['min_move_deposit'], each_c['min_gas_tx'], each_c['fee_limit'], each_c['gas_ticker'], each_c['move_gas_amount'], each_c['chain_id'], each_c['real_deposit_fee'], 7200 )
                        pass
                    except Exception as e:
                        traceback.print_exc(file=sys.stdout)
            ...
            if len(main_tokens) > 0:
                for each_c in main_tokens:
                    try:
                        type_name = each_c['type']
                        await store.trx_check_minimum_deposit(each_c['coin_name'], type_name, None, each_c['decimal'], each_c['min_move_deposit'], each_c['min_gas_tx'], each_c['fee_limit'], each_c['gas_ticker'], each_c['move_gas_amount'], each_c['chain_id'], each_c['real_deposit_fee'], 7200 )
                        pass
                    except Exception as e:
                        traceback.print_exc(file=sys.stdout)
        except Exception as e:
    ...
    async def trtl_service_getTransactions(self, url: str, coin: str, firstBlockIndex: int=2000000, blockCount: int= 200000):
        ...
        result = await self.wallet_api.call_aiohttp_wallet_xmr_bcn('getTransactions', COIN_NAME, time_out=time_out, payload=payload)
    ...
    async def sql_get_userwallet(self, userID, coin: str, netname: str, type_coin: str, user_server: str = 'DISCORD', chat_id: int = 0):
        return await self.wallet_api.sql_get_userwallet(userID, coin, netname, type_coin, user_server, chat_id)


    async def sql_register_user(self, userID, coin: str, netname: str, type_coin: str, user_server: str, chat_id: int = 0, is_discord_guild: int=0):
        return await self.wallet_api.sql_register_user(userID, coin, netname, type_coin, user_server, chat_id, is_discord_guild)
                ...
                if type_coin in ["ERC-20"]:
                    ...
                    if not valid_address == True:
                        ...
                        if ctx.author.id not in self.bot.TX_IN_PROCESS:
                            ...
                            try:
                                main_address = getattr(getattr(self.bot.coin_list, COIN_NAME), "MainAddress")
                                SendTx = await self.wallet_api.send_external_nano(main_address, str(ctx.author.id), amount, address, COIN_NAME, coin_decimal)
                                if SendTx:
                                    fee_txt = "\nWithdrew fee/node: `0.00 {}`.".format(COIN_NAME)
                                    SendTx_hash = SendTx['block']
                                    msg = f'{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention}, you withdrew {num_format_coin(amount, COIN_NAME, coin_decimal, False)} {token_display}{equivalent_usd} to `{address}`.\nTransaction hash: `{SendTx_hash}`{fee_txt}'
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        self.bot.TX_IN_PROCESS.append(ctx.author.id)
                        SendTx = await self.wallet_api.send_external_xch(str(ctx.author.id), amount, address, COIN_NAME, coin_decimal, tx_fee, NetFee, SERVER_BOT)
                        if SendTx:
                            fee_txt = "\nWithdrew fee/node: `{} {}`.".format(num_format_coin(NetFee, COIN_NAME, coin_decimal, False), COIN_NAME)
                            await logchanbot(f'A user {ctx.author.name}#{ctx.author.discriminator} / {ctx.author.mention} successfully withdrew {num_format_coin(amount, COIN_NAME, coin_decimal, False)} {token_display}{equivalent_usd}.')
                            msg = f'{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention}, you withdrew {num_format_coin(amount, COIN_NAME, coin_decimal, False)} {token_display}{equivalent_usd} to `{address}`.\nTransaction hash: `{SendTx}`{fee_txt}'
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        if COIN_NAME == "ADA":
                            ...
                            fee_limit = getattr(getattr(self.bot.coin_list, COIN_NAME), "fee_limit")
                            # Use fee limit as NetFee
                            SendTx = await self.wallet_api.send_external_ada(str(ctx.author.id), amount, coin_decimal, SERVER_BOT, COIN_NAME, fee_limit, address, 60)
                            if "status" in SendTx and SendTx['status'] == "pending":
                                tx_hash = SendTx['id']
                                fee = SendTx['fee']['quantity']/10**coin_decimal + fee_limit
                                fee_txt = "\nWithdrew fee/node: `{} {}`.".format(num_format_coin(fee, COIN_NAME, coin_decimal, False), COIN_NAME)
                            ...
                            asset_name = getattr(getattr(self.bot.coin_list, COIN_NAME), "header")
                            policy_id = getattr(getattr(self.bot.coin_list, COIN_NAME), "contract")
                            SendTx = await self.wallet_api.send_external_ada_asset(str(ctx.author.id), amount, coin_decimal, SERVER_BOT, COIN_NAME, NetFee, address, asset_name, policy_id, 60)
                            if "status" in SendTx and SendTx['status'] == "pending":
                                tx_hash = SendTx['id']
                                gas_coin_msg = ""
                                if GAS_COIN is not None:
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        self.bot.TX_IN_PROCESS.append(ctx.author.id)
                        tx_fee = getattr(getattr(self.bot.coin_list, COIN_NAME), "tx_fee")
                        SendTx = await self.wallet_api.send_external_sol(self.bot.erc_node_list['SOL'], str(ctx.author.id), amount, address, COIN_NAME, coin_decimal, tx_fee, NetFee, SERVER_BOT)
                        if SendTx:
                            fee_txt = "\nWithdrew fee/node: `{} {}`.".format(num_format_coin(NetFee, COIN_NAME, coin_decimal, False), COIN_NAME)
                            msg = f'{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention}, you withdrew {num_format_coin(amount, COIN_NAME, coin_decimal, False)} {token_display}{equivalent_usd} to `{address}`.\nTransaction hash: `{SendTx}`{fee_txt}'
                            await ctx.edit_original_message(content=msg)
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        self.bot.TX_IN_PROCESS.append(ctx.author.id)
                        SendTx = await self.wallet_api.send_external_doge(str(ctx.author.id), amount, address, COIN_NAME, 0, NetFee, SERVER_BOT) # tx_fee=0
                        if SendTx:
                            fee_txt = "\nWithdrew fee/node: `{} {}`.".format(num_format_coin(NetFee, COIN_NAME, coin_decimal, False), COIN_NAME)
                            msg = f'{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention}, you withdrew {num_format_coin(amount, COIN_NAME, coin_decimal, False)} {token_display}{equivalent_usd} to `{address}`.\nTransaction hash: `{SendTx}`{fee_txt}'
                            await ctx.edit_original_message(content=msg)
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        ...
                        header = getattr(getattr(self.bot.coin_list, COIN_NAME), "header")
                        is_fee_per_byte = getattr(getattr(self.bot.coin_list, COIN_NAME), "is_fee_per_byte")
                        SendTx = await self.wallet_api.send_external_xmr(type_coin, main_address, str(ctx.author.id), amount, address, COIN_NAME, coin_decimal, tx_fee, NetFee, is_fee_per_byte, mixin, SERVER_BOT, wallet_address, header, None) # paymentId: None (end)
                        if SendTx:
                            fee_txt = "\nWithdrew fee/node: `{} {}`.".format(num_format_coin(NetFee, COIN_NAME, coin_decimal, False), COIN_NAME)
                            msg = f'{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention}, you withdrew {num_format_coin(amount, COIN_NAME, coin_decimal, False)} {token_display}{equivalent_usd} to `{address}`.\nTransaction hash: `{SendTx}`{fee_txt}'
                            await ctx.edit_original_message(content=msg)
        ...
        try:
            if not info:
                ...
                if usd_equivalent_enable == 1:
                    ...
                    if per_unit and per_unit > 0:
                        amount_in_usd = float(Decimal(per_unit) * Decimal(amount))
                tip = await store.sql_user_balance_mv_single(str(self.bot.user.id), str(ctx.author.id), str(ctx.guild.id), str(ctx.channel.id), amount, COIN_NAME, "FAUCET", coin_decimal, SERVER_BOT, contract, amount_in_usd, None)
                try:
    ...
    async def donate(
        ...
        if ctx.author.id not in self.bot.TX_IN_PROCESS:
            self.bot.TX_IN_PROCESS.append(ctx.author.id)
            try:
                donate = await store.sql_user_balance_mv_single(str(ctx.author.id), str(self.donate_to), "DONATE", "DONATE", amount, COIN_NAME, "DONATE", coin_decimal, SERVER_BOT, contract, amount_in_usd, None)
                if donate:
    ...
    async def purchase(
        ...
        if check_list is None:
            ...
            try:
                ...
                get_deposit = await self.wallet_api.sql_get_userwallet(str(ctx.author.id), FROM_COIN, net_name, type_coin, SERVER_BOT, 0)
                ...
                if amount < MinTip or amount > MaxTip:
                    ...
                    return
                if get_deposit is None:
                    get_deposit = await self.wallet_api.sql_register_user(str(ctx.author.id), FROM_COIN, net_name, type_coin, SERVER_BOT, 0, 1 if check_list['is_guild']==1 else 0)
                wallet_address = get_deposit['balance_wallet_address']
                if type_coin in ["TRTL-API", "TRTL-SERVICE", "BCN", "XMR"]:
                    wallet_address = get_deposit['paymentid']
                ...
                get_deposit = await self.wallet_api.sql_get_userwallet(creditor, TO_COIN, net_name, type_coin, SERVER_BOT, 0)
                ...
                if get_deposit is None:
                    get_deposit = await self.wallet_api.sql_register_user(creditor, TO_COIN, net_name, type_coin, SERVER_BOT, 0, 1 if check_list['is_guild']==1 else 0)
                wallet_address = get_deposit['balance_wallet_address']
                if type_coin in ["TRTL-API", "TRTL-SERVICE", "BCN", "XMR"]:
                    wallet_address = get_deposit['paymentid']
    ...
    async def swap(
        ...
        if PAIR_NAME not in self.swap_pair:
            ...
            if amount is None:
                ...
                get_deposit = await self.wallet_api.sql_get_userwallet(str(ctx.author.id), FROM_COIN, net_name, type_coin, SERVER_BOT, 0)
                if get_deposit is None:
                    get_deposit = await self.wallet_api.sql_register_user(str(ctx.author.id), FROM_COIN, net_name, type_coin, SERVER_BOT, 0, 0)
