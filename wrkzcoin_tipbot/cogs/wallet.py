                    ...
                    if 'balances' in account and len(account['balances']) > 0:
                        for each_balance in account['balances']:
                            if coin_name == "XLM" and each_balance['asset_type'] == "native":
                                ...
                                break
                            elif 'asset_code' in each_balance and 'asset_issuer' in each_balance and "{}XLM".format(
                                    each_balance['asset_code']) == coin_name and issuer == each_balance['asset_issuer']:
                                balance = float(each_balance['balance'])
                                break
            except Exception:
                traceback.print_exc(file=sys.stdout)
    ...
    async def check_xlm_asset(self, url: str, asset_name: str, issuer: str, to_address: str, user_id: str,
        ...
        try:
            async with ServerAsync(
                ...
                if 'balances' in account and len(account['balances']) > 0:
                    for each_balance in account['balances']:
                        if 'asset_code' in each_balance and 'asset_issuer' in each_balance and "{}XLM".format(
...
class Wallet(commands.Cog):
                                    ...
                                    if not hasattr(self.bot.coin_list, coin_name) or getattr(
                                        ...
                                        try:
                                            ...
                                            if not amount.isdigit() and amount.upper() == "ALL":
                                                all_amount = True
                                                userdata_balance = await self.user_balance(each_msg['sender_id'],
                                            ...
                                            if type_coin in ["ERC-20"]:
                                                ...
                                                try:
                                                    ...
                                                    chain_id = getattr(getattr(self.bot.coin_list, coin_name),
                                                                       "chain_id")
                                                    SendTx = await self.send_external_erc20(url, net_name,
                                                                                            each_msg['sender_id'],
                                                ...
                                                SendTx = None
                                                try:
                                                    SendTx = await self.send_external_trc20(each_msg['sender_id'],
                                                                                            address, amount, coin_name,
                                                                                            coin_decimal, NetFee,
                                                ...
                                                if not valid_address is True:
                                                    ...
                                                    try:
                                                        main_address = getattr(getattr(self.bot.coin_list, coin_name),
                                                                               "MainAddress")
                                                        SendTx = await self.wallet_api.send_external_nano(main_address,
                                                                                                          each_msg[
                                                                                                              'sender_id'],
                                                                                                          amount,
                                                                                                          address,
                                                ...
                                                SendTx = await self.wallet_api.send_external_xch(each_msg['sender_id'],
                                                                                                 amount, address,
                                                                                                 coin_name,
                                                                                                 coin_decimal, tx_fee,
                                                                                                 NetFee, user_server)
                                                ...
                                                SendTx = await self.wallet_api.send_external_hnt(each_msg['sender_id'],
                                                ...
                                                if coin_name == "ADA":
                                                    ...
                                                    SendTx = await self.wallet_api.send_external_ada(
                                                        ...
                                                        if getattr(getattr(self.bot.coin_list, coin_name),
                                                            ...
                                                            if GAS_COIN:
                                                                userdata_balance = await self.user_balance(
                                                                    ...
                                                                    type_coin, height,
                                                                    getattr(getattr(self.bot.coin_list, GAS_COIN),
                                                    ...
                                                    policy_id = getattr(getattr(self.bot.coin_list, coin_name),
                                                                        "contract")
                                                    SendTx = await self.wallet_api.send_external_ada_asset(
                                                        each_msg['sender_id'], amount, coin_decimal, user_server,
                                                    ...
                                                    if "status" in SendTx and SendTx['status'] == "pending":
                                                        ...
                                                        if GAS_COIN is not None:
                                                            gas_coin_msg = " and fee `{} {}` you shall receive additional `{} {}`.".format(
                                                                num_format_coin(SendTx['network_fee'] + fee_limit / 20,
                                                                                GAS_COIN, 6, False), GAS_COIN,
                                                                num_format_coin(SendTx['ada_received'], GAS_COIN, 6,
                                                                                False), GAS_COIN)
                                                        fee_txt = "\nWithdrew fee/node: `{} {}`{}.".format(
                                                            num_format_coin(NetFee, coin_name, coin_decimal, False),
                                                ...
                                                tx_fee = getattr(getattr(self.bot.coin_list, coin_name), "tx_fee")
                                                SendTx = await self.wallet_api.send_external_sol(
                                                ...
                                                SendTx = await self.wallet_api.send_external_doge(each_msg['sender_id'],
                                                                                                  amount, address,
                                                                                                  coin_name, 0, NetFee,
                                                ...
                                                is_fee_per_byte = getattr(getattr(self.bot.coin_list, coin_name),
                                                                          "is_fee_per_byte")
                                                SendTx = await self.wallet_api.send_external_xmr(type_coin,
                                                                                                 main_address,
                                                                                                 each_msg['sender_id'],
                                                                                                 amount, address,
                                                                                                 coin_name,
    ...
    async def async_balance(self, ctx, token: str = None):
        ...
        try:
            ...
            try:
                # height can be None
                userdata_balance = await self.user_balance(str(ctx.author.id), coin_name, wallet_address, type_coin,
                ...
                if type_coin in ["ERC-20"]:
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        ...
                        try:
                            ...
                            SendTx = await self.send_external_erc20(url, net_name, str(ctx.author.id), address, amount,
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        self.bot.TX_IN_PROCESS.append(ctx.author.id)
                        try:
                            SendTx = await self.send_external_trc20(str(ctx.author.id), address, amount, coin_name,
                    ...
                    if not valid_address is True:
                        ...
                        if ctx.author.id not in self.bot.TX_IN_PROCESS:
                            ...
                            try:
                                main_address = getattr(getattr(self.bot.coin_list, coin_name), "MainAddress")
                                SendTx = await self.wallet_api.send_external_nano(main_address, str(ctx.author.id),
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        self.bot.TX_IN_PROCESS.append(ctx.author.id)
                        SendTx = await self.wallet_api.send_external_xch(str(ctx.author.id), amount, address, coin_name,
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        ...
                        SendTx = await self.wallet_api.send_external_hnt(str(ctx.author.id), wallet_host, password,
                    ...
                    if coin_name != "XLM":  # in case of asset
                        issuer = getattr(getattr(self.bot.coin_list, coin_name), "contract")
                        check_asset = await self.wallet_api.check_xlm_asset(url, coin_name, issuer, address,
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        ...
                        SendTx = await self.wallet_api.send_external_xlm(url, withdraw_keypair, str(ctx.author.id),
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        if coin_name == "ADA":
                            ...
                            fee_limit = getattr(getattr(self.bot.coin_list, coin_name), "fee_limit")
                            # Use fee limit as NetFee
                            SendTx = await self.wallet_api.send_external_ada(str(ctx.author.id), amount, coin_decimal,
                            ...
                            try:
                                if getattr(getattr(self.bot.coin_list, coin_name), "withdraw_use_gas_ticker") == 1:
                                    ...
                                    fee_limit = getattr(getattr(self.bot.coin_list, coin_name), "fee_limit")
                                    if GAS_COIN:
                                        userdata_balance = await self.user_balance(str(ctx.author.id), GAS_COIN,
                                                                                   wallet_address, type_coin, height,
                                                                                   getattr(getattr(self.bot.coin_list,
                                                                                                   GAS_COIN),
                                                                                           "deposit_confirm_depth"),
                                                                                   SERVER_BOT)
                                        actual_balance = userdata_balance['adjust']
                            ...
                            SendTx = await self.wallet_api.send_external_ada_asset(str(ctx.author.id), amount,
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        self.bot.TX_IN_PROCESS.append(ctx.author.id)
                        tx_fee = getattr(getattr(self.bot.coin_list, coin_name), "tx_fee")
                        SendTx = await self.wallet_api.send_external_sol(self.bot.erc_node_list['SOL'],
                    ...
                    if ctx.author.id not in self.bot.TX_IN_PROCESS:
                        self.bot.TX_IN_PROCESS.append(ctx.author.id)
                        SendTx = await self.wallet_api.send_external_doge(str(ctx.author.id), amount, address,
    ...
    async def bot_faucet(
        ...
        for coin_name in [coinItem.upper() for coinItem in faucet_coins]:
            ...
            height = self.wallet_api.get_block_height(type_coin, coin_name, net_name)
            try:
                userdata_balance = await self.user_balance(str(self.bot.user.id), coin_name, wallet_address, type_coin,
                                                           height, deposit_confirm_depth, SERVER_BOT)
                actual_balance = float(userdata_balance['adjust'])
    ...
    async def donate(
        ...
        if not amount.isdigit() and amount.upper() == "ALL":
            all_amount = True
            userdata_balance = await self.user_balance(str(ctx.author.id), coin_name, wallet_address, type_coin, height,
                                                       deposit_confirm_depth, SERVER_BOT)
    ...
    async def swap(
        ...
        if PAIR_NAME not in self.swap_pair:
            ...
            if amount is None:
                ...
                height = self.wallet_api.get_block_height(type_coin, FROM_COIN, net_name)
                userdata_balance = await self.user_balance(str(ctx.author.id), FROM_COIN, wallet_address, type_coin,
                                                           height, deposit_confirm_depth, SERVER_BOT)
                actual_balance = float(userdata_balance['adjust'])
