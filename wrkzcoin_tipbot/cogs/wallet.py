...
class WalletAPI(commands.Cog):
    ...
    def near_move_balance_token(
        self, url: str, contract_id: str, key: str, by_address: str, to_address: str, atomic_amount: int
    ):
        try:
            near_provider = near_api.providers.JsonProvider(url)
            sender_key_pair = near_api.signer.KeyPair(bytes.fromhex(key))
    ...
    async def near_insert_mv_balance(
        ...
        token_decimal: int, txn: str, content: str, time_insert: int, user_server: str, network: str
    ):
        try:
            await self.openConnection()
            async with self.pool.acquire() as conn:
    ...
    async def send_external_near(
        self, url: str, contract_id: str, key: str, user_from: str, by_address: str,
        amount: float, to_address: str, coin: str, coin_decimal: int, withdraw_fee: float, user_server: str = 'DISCORD'
    ):
        try:
    ...
    def zil_transfer_token(self, contract_addr: str, to_address: str, from_key: str, atomic_amount: int):
        try:
            ...
            resp = contract.call(
                method="Transfer",
                params=[zil_contract.value_dict("to", "ByStr20", to_account.address0x), zil_contract.value_dict("amount", "Uint128", str(atomic_amount))]
            )
            return resp
        except Exception:
            traceback.print_exc(file=sys.stdout)
                                ...
                                try:
                                    ...
                                    for coin_name in all_names:
                                        ...
                                        if total_balance == 0:
                                            ...
                                            non_zero_tokens[coin_name] = num_format_coin(
                                                total_balance, coin_name,
                                                coin_decimal,
                                                False
                                            ) + " " + token_display
                                            ...
                                            if type_coin in ["ERC-20"]:
                                                ...
                                                if send_tx:
                                                    fee_txt = "\nWithdrew fee/node: `{} {}`.".format(
                                                        num_format_coin(NetFee, coin_name, coin_decimal, False),
                                                        coin_name)
                                                    response = f"You withdrew {num_format_coin(amount, coin_name, coin_decimal, False)} "\
                        ...
                        for eachTx in result:
                            if eachTx['user_id']:
                                ...
                                if member is not None:
                                    coin_decimal = getattr(getattr(self.bot.coin_list, eachTx['coin_name']), "decimal")
                                    msg = "You got a new deposit (it could take a few minutes to credit): ```" + "Coin: {}\nTx: {}\nAmount: {}".format(
                                        eachTx['coin_name'], eachTx['txid'], num_format_coin(eachTx['amount'], eachTx['coin_name'], coin_decimal, False
                                        )
                                    ) + "```"
                                    try:
                                        await member.send(msg)
                                        sql = """ UPDATE `cn_get_transfers` SET `notified_confirmation`=%s, `time_notified`=%s WHERE `txid`=%s LIMIT 1 """
                                        await cur.execute(sql, ("YES", int(time.time()), eachTx['txid']))
                                        await conn.commit()
                                    except Exception:
                                        traceback.print_exc(file=sys.stdout)
                                        sql = """ UPDATE `cn_get_transfers` SET `notified_confirmation`=%s, `failed_notification`=%s WHERE `txid`=%s LIMIT 1 """
                                        await cur.execute(sql, ("NO", "YES", eachTx['txid']))
                                        await conn.commit()
        except Exception:
            traceback.print_exc(file=sys.stdout)
                        ...
                        for eachTx in result:
                            if eachTx['user_id']:
                                ...
                                if member is not None:
                                    ...
                                    try:
                                        await member.send(msg)
                                        sql = """ UPDATE `doge_get_transfers` SET `notified_confirmation`=%s, `time_notified`=%s 
                                        WHERE `txid`=%s LIMIT 1 """
                                        await cur.execute(sql, ("YES", int(time.time()), eachTx['txid']))
                                        await conn.commit()
                                    except Exception:
                                        traceback.print_exc(file=sys.stdout)
                                        sql = """ UPDATE `doge_get_transfers` SET `notified_confirmation`=%s, `failed_notification`=%s 
                                        WHERE `txid`=%s LIMIT 1 """
                                        await cur.execute(sql, ("NO", "YES", eachTx['txid']))
                                        await conn.commit()
        except Exception:
                        ...
                        for eachTx in result:
                            if eachTx['user_id']:
                                ...
                                if member is not None:
                                    ...
                                    try:
                                        await member.send(msg)
                                        sql = """ UPDATE `xch_get_transfers` SET `notified_confirmation`=%s, `time_notified`=%s 
                                        WHERE `txid`=%s LIMIT 1 """
                                        await cur.execute(sql, ("YES", int(time.time()), eachTx['txid']))
                                        await conn.commit()
                                    except Exception:
                                        traceback.print_exc(file=sys.stdout)
                                        sql = """ UPDATE `xch_get_transfers` SET `notified_confirmation`=%s, `failed_notification`=%s 
                                        WHERE `txid`=%s LIMIT 1 """
                                        await cur.execute(sql, ("NO", "YES", eachTx['txid']))
                                        await conn.commit()
        except Exception:
                        ...
                        for eachTx in result:
                            if eachTx['user_id']:
                                ...
                                if member is not None:
                                    coin_decimal = getattr(getattr(self.bot.coin_list, eachTx['coin_name']), "decimal")
                                    msg = "You got a new deposit (it could take a few minutes to credit): ```" + "Coin: {}\nBlock: {}\nAmount: {}".format(
                                        eachTx['coin_name'], eachTx['block'], num_format_coin(eachTx['amount'], eachTx['coin_name'], coin_decimal, False
                                        )
                                    ) + "```"
                                    try:
                                        await member.send(msg)
                                        sql = """ UPDATE `nano_move_deposit` SET `notified_confirmation`=%s, `time_notified`=%s WHERE `block`=%s LIMIT 1 """
                                        await cur.execute(sql, ("YES", int(time.time()), eachTx['block']))
                                        await conn.commit()
                                    except Exception:
                                        traceback.print_exc(file=sys.stdout)
                                        sql = """ UPDATE `nano_move_deposit` SET `notified_confirmation`=%s, `failed_notification`=%s WHERE `block`=%s LIMIT 1 """
                                    ...
                                    try:
                                        ...
                                        if move_to_deposit:
                                            try:
                                                ...
                                                async with self.pool.acquire() as conn:
                                                    async with conn.cursor() as cur:
                                                        sql = """ INSERT INTO nano_move_deposit (`coin_name`, `user_id`, `balance_wallet_address`, 
                                                        `to_main_address`, `amount`, `decimal`, `block`, `time_insert`, `user_server`) 
                                                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                                                        """
                                                        await cur.execute(sql, (
                                                            coin_name, all_deposit_address[address]['user_id'], address, main_address,
                                                            float(int(balance['balance']) / 10 ** coin_decimal), coin_decimal, 
                                                            move_to_deposit['block'], int(time.time()), all_deposit_address[address]['user_server']
                                                        ))
                                                        await conn.commit()
                                                        updated += 1
                                            except Exception:
            ...
            async with self.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    if main_token is False:
                        sql = """ SELECT * FROM `coin_settings` WHERE `type`=%s AND `enable`=%s 
            ...
            if height is None:
                ...
                if amount + NetFee > actual_balance:
                    ...
                    msg = f"{EMOJI_RED_NO} {ctx.author.mention}, transaction cannot be smaller than "\
                        f"{num_format_coin(min_tx, coin_name, coin_decimal, False)} {token_display} or "\
                        f"bigger than {num_format_coin(max_tx, coin_name, coin_decimal, False)} {token_display}."
                    await ctx.edit_original_message(content=msg)
                    return
    ...
    async def take_action(
        ...
        try:
            if not info:
                ...
                try:
                    faucet_add = await store.sql_faucet_add(
                    ...
                    await logchanbot(
                        ...
                        f"claimed faucet {num_format_coin(amount, coin_name, coin_decimal, False)} {coin_name}"\
    ...
    async def donate(
        ...
        if amount <= 0 or actual_balance <=0:
            ...
            msg = f"{EMOJI_RED_NO} {ctx.author.mention}, transactions (donate) cannot be bigger than "\
                f"**{num_format_coin(max_tip * donate_factor, coin_name, coin_decimal, False)} {token_display}** "\
                f"or smaller than **{num_format_coin(min_tip / donate_factor, coin_name, coin_decimal, False)} {token_display}**."
            await ctx.edit_original_message(content=msg)
            return
        ...
        if ctx.author.id not in self.bot.TX_IN_PROCESS:
            ...
            try:
                ...
                if donate:
                    ...
                    await ctx.edit_original_message(content=msg)
                    await logchanbot(
                        f"[DONATE] A user {ctx.author.name}#{ctx.author.discriminator} / {ctx.author.mention} "\
                        f"donated {num_format_coin(amount, coin_name, coin_decimal, False)} "\
                        f"{token_display}{equivalent_usd}."
                    )
            except Exception:
                traceback.print_exc(file=sys.stdout)
            ...
            try:
                ...
                if actual_balance < amount:
                    msg = f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to /swaptokens "\
                        f"**{num_format_coin(amount, from_coin, coin_decimal, False)} {token_display}**. "\
                        f"Having {num_format_coin(actual_balance, from_coin, coin_decimal, False)} {token_display}."
                    await ctx.edit_original_message(content=msg)
                    return
                ...
                if actual_balance < amount_swapped:
                    msg = f"{EMOJI_RED_NO} {ctx.author.mention}, creditor has insufficient balance to /swaptokens "\
                        f"**{num_format_coin(amount_swapped, from_coin, coin_decimal, False)} {token_display}**. "\
                        f"Remaining only {num_format_coin(actual_balance, to_coin, coin_decimal, False)} {token_display}."
                    await ctx.edit_original_message(content=msg)
                    try:
                        msg_log = f"[SWAPTOKENS] - A user {ctx.author.name}#{ctx.author.discriminator} / "\
                            f"{ctx.author.id}  /swaptokens failed! Shortage of creditor's balance. "\
                            f"Remaining only {num_format_coin(actual_balance, to_coin, coin_decimal, False)} "\
                            f"{token_display}."
                    ...
                    if swap > 0:
                        ...
                        try:
                            msg_log = f"[SWAPTOKENS] - A user {ctx.author.name}#{ctx.author.discriminator} "\
                                ...
                                f"{num_format_coin(amount, from_coin, token_decimal_from, False)} {from_coin} "\
                                f"to {num_format_coin(amount_swapped, to_coin, token_decimal_to, False)} {to_coin} "\
                                f"[Fee: {num_format_coin(fee_to, to_coin, token_decimal_to, False)} {to_coin}].\n"\
                                f"CREDITOR ID: {creditor} NEW BALANCE: "\
                                f"{num_format_coin(float(creditor_balance['adjust']) - amount_swapped, to_coin, token_decimal_to, False)}"\
                                f" {to_coin}."
                            await logchanbot(msg_log)
                            if check_list['channel_log'] and check_list['channel_log'].isdigit():
                                self.logchan_swap = self.bot.get_channel(int(check_list['channel_log']))
                        ...
                        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, successfully /swaptokens from "\
                            f"**{num_format_coin(amount, from_coin, token_decimal_from, False)} {from_coin}** to "\
                            f"**{num_format_coin(amount_swapped, to_coin, token_decimal_to, False)}** _{to_coin} "\
                            f"[Fee: {num_format_coin(fee_to, to_coin, token_decimal_to, False)} {to_coin}]_. Thanks!"
                        await ctx.edit_original_message(content=msg)
                    else:
            ...
            if amount is None:
                ...
                if amount > max_tip or amount < min_tip:
                    msg = f"{EMOJI_RED_NO} {ctx.author.mention}, swap cannot be bigger than "\
                        f"**{num_format_coin(max_tip, from_coin, coin_decimal, False)} {token_display}** "\
                        f"or smaller than **{num_format_coin(min_tip, from_coin, coin_decimal, False)} {token_display}**."
                    await ctx.edit_original_message(content=msg)
                    return
                elif amount > actual_balance:
                ...
                try:
                    ...
                    if balance / 5 < to_amount:  # We allow 20% to swap
                        ...
                        await logchanbot(
                            ...
                            f"swap from `{num_format_coin(amount, from_coin, coin_decimal, False)} {from_coin} "\
                            f"to {num_format_coin(to_amount, to_coin, to_coin_decimal, False)} {to_coin}` "\
                            f"but shortage of liquidity. "\
                            f"Having only `{num_format_coin(balance, to_coin, to_coin_decimal, False)} {to_coin}`."
                        )
                        return
                except Exception:
                    traceback.print_exc(file=sys.stdout)
                    ...
                    try:
                        ...
                        if swap:
                            msg = f"{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention}, swapped from "\
                                f"`{num_format_coin(amount, from_coin, coin_decimal, False)} "\
                                f"{from_coin} to {num_format_coin(to_amount, to_coin, to_coin_decimal, False)} {to_coin}`."
                            await ctx.edit_original_message(content=msg)
                            await logchanbot(
                                f"A user {ctx.author.name}#{ctx.author.discriminator} / {ctx.author.mention} "\
                                f"swapped from `{num_format_coin(amount, from_coin, coin_decimal, False)} {from_coin} "\
                                f"to {num_format_coin(to_amount, to_coin, to_coin_decimal, False)} {to_coin}`."
                            )
                    except Exception:
                        traceback.print_exc(file=sys.stdout)
