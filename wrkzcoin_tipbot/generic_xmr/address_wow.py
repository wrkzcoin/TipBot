...
_ADDR_REGEX = re.compile(r'^Wo[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{95}$')
_SADDR_REGEX = re.compile(r'^WW[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{95}$')
_IADDR_REGEX = re.compile(r'^So[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{106}$')
...
class Address(BaseAddress):
    ...
    :param address: a Wow address as string-like object
    :param label: a label for the address (defaults to `None`)
    """
    _valid_netbytes = (4146, 53, 24)
    ...
    def with_payment_id(self, payment_id=0):
        ...
        :param payment_id: int, hexadecimal string or :class:`PaymentID <wownero.numbers.PaymentID>`
                    (max 64-bit long)
        ...
        prefix = 53 if self.is_testnet() else 24 if self.is_stagenet() else 4146
...
class IntegratedAddress(Address):
    ...
    def payment_id(self):
        ...
        :rtype: :class:`PaymentID <wownero.numbers.PaymentID>`
        """
    ...
    def base_address(self):
        ...
        :rtype: :class:`Address`
        """
        prefix = 24 if self.is_testnet() else 53 if self.is_stagenet() else 4146
        data = bytearray(unhexlify('%02x' % prefix)) + self._decoded[4:68]
        checksum = keccak_256(data).digest()[:4]
        return Address(base58.encode(hexlify(data + checksum)))


def address_wow(addr, label=None):
    """Discover the proper class and return instance for a given Wownero address.
