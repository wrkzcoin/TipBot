...
class Address(BaseAddress):
    ...
    def with_payment_id(self, payment_id=0):
        ...
        prefix = 252 if self.is_testnet() else 253 if self.is_stagenet() else 251
...
class IntegratedAddress(Address):
    ...
    def base_address(self):
        ...
        :rtype: :class:`Address`
        """
        prefix = 186 if self.is_testnet() else 187 if self.is_stagenet() else 185
        data = bytearray([prefix]) + self._decoded[1:65]
        checksum = keccak_256(data).digest()[:4]
        return Address(base58.encode(hexlify(data + checksum)))


def address_xol(addr, label=None):
