                ...
                async with conn.cursor() as cur:
                    ...
                    if len(get_dm) > 0:
                        ...
                        for each_msg in get_dm:
                            if len(existing_msg_ids) == 0 or (len(existing_msg_ids) > 0 and each_msg['id'] not in existing_msg_ids):
                                # Skip its own message
                                if each_msg['message_create']['sender_id'] != bot_id:
                                    data_rows.append( ( json.dumps(each_msg), json.dumps(each_msg['message_create']['message_data']), each_msg['message_create']['message_data']['text'], each_msg['id'], each_msg['message_create']['sender_id'], each_msg['message_create']['target']['recipient_id'], int(int(each_msg['created_timestamp'])/1000), fetched_at ))
                        # insert to DB
                        if len(data_rows) > 0:
